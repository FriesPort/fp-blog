...
```bash
sudo mount -t nfs -o rw,nolock <storage-ip>:<storage-path> /mnt/nfs
```
...
```sh
<storage-ip>:<storage-path>  <cal-path>  nfs  rw,tcp,nolock,soft,timeo=30,retrans=2,_netdev  0  0
```
...
volumes:
  teable-nfs:
    ...
    driver_opts:
      type: nfs
      o: "addr=<calculate-ip>,rw,nolock,soft,nfsvers=3,noresvport"
